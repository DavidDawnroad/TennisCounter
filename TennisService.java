	...
	public void recordCreate(TennisDTO dto, TennisDAO dao) {
		...
		while(loop) {
			...
			if (dao.player.size() == 2) {
				...
				checkRalleyPointVisible();
				System.out.printf("%s, %s(%s) VS %s, %s(%s)\n",
	...
	public void startRalley(TennisDTO dto, TennisDAO dao) {
		if (dao.player.size() == 2) {
			if (point == 0) { // 좌측 팀 득점
				...
				if (ralleyPointA == 3 && ralleyPointB == 3) {   // 둘 다 40점에 도달하면
				...
				} else if (ralleyPointA == 3) { // A만 40점이고 B가 아직 40점에 도달 못 했으면
					if (ralleyPointB < 3) {
						winningStreakA++;
						winningStreakB = 0;
						if (winningStreakA == 2) { 	 				// 2연승이면
...
/*
 	2024-07-22
 	gamepoint 변수가 초기화되지 않고 계속 6점으로 남아있어서 2세트부터 랠리가 스킵되는 문제를 resetgames()를 다시 만들어 적용하는 것으로 해결
 	3선승제에서 2세트를 땄을 경우 isMatchEnds()가 제대로 돌아가게 만들면 scoreCreate 끝.
 */
	void isMatchEnds(TennisDTO dto, TennisDAO dao) {
		
		
		if (dao.sets == 3) { // 3세트전 경기일 경우
			if (dao.matchType.equals("단식")) {
			...
			} else if (dao.matchType.equals("복식")) {
				if (setPointA == 2) { // A팀이 승리
					System.out.println("계수가 종료되었습니다.");
...
//여기 수정					
				System.out.printf("Tiebreak의 %d번째 랠리에서 %s이(가) 득점했다면 0을, %s이(가) 득점했다면 1을 입력해주세요.: "
						  , ralleyCount, dto.getPlayerList().get(0), dto.getPlayerList().get(1));
			} else if (dao.player.size() == 4) {
				checkRalleyPointVisible();
//여기 수정	
				System.out.printf("Tiebreak의 %d번째 랠리에서 %s, %s팀이 득점했다면 0을, %s, %s팀이 득점했다면 1을 입력해주세요.: "
						, ralleyCount, dto.getPlayerList().get(0), dto.getPlayerList().get(1), dao.player.get(2), dao.player.get(3));
			}
			point = scan.nextInt();
			...
			try {
				...
				if (point == 0) {
					System.out.printf("%s가 득점하였습니다.",dao.player.get(0));
					ralleyPointA++;
					dto.setRalleyPointA(ralleyPointA);
//					ralleyCount++;
				
				}
				else if (point == 1){
					System.out.printf("%s가 득점하였습니다.",dao.player.get(1));
					ralleyPointB++;
					dto.setRalleyPointB(ralleyPointB);
//					ralleyCount++;
				...
				System.out.println();
				//현재 점수 현황
				System.out.printf("[현재 점수] %d : %d\n", dto.getRalleyPointA(), dto.getRalleyPointB()); 
				System.out.println();
				...
				if((dto.getRalleyPointA() >= 7 || dto.getRalleyPointB() >= 7) && (Math.abs(dto.getRalleyPointA() - dto.getRalleyPointB()) >= 2)) { //abs절댓값 표현 //==7하면 7점이 될때까지 끝나지 않음
					if (dto.getMatchType() == "단식") {
						if (dto.getRalleyPointA() > dto.getRalleyPointB()) {
							System.out.printf("%s가 타이브레이크 게임에서 승리하여 %d세트의 승자가 되었습니다.",dto.getPlayerList().get(0), setCount);
							setCount++;
							setPointA++;
						} else if (dto.getRalleyPointA() < dto.getRalleyPointB()) {
							System.out.printf("%s가 타이브레이크 게임에서 승리하여 %d세트의 승자가 되었습니다.",dto.getPlayerList().get(1), setCount);
							setCount++;
							setPointB++;
						}
					} else if (dto.getMatchType() == "복식") {
						if (dto.getRalleyPointA() > dto.getRalleyPointB()) {
							System.out.printf("%s, %s가 타이브레이크 게임에서 승리하여 %d세트의 승자팀이 되었습니다."
											  ,dto.getPlayerList().get(0), dto.getPlayerList().get(1), setCount);
							setCount++;
							setPointA++;
						} else if (dto.getRalleyPointA() < dto.getRalleyPointB()) {
							System.out.printf("%s, %s가 타이브레이크 게임에서 승리하여 %d세트의 승자팀이 되었습니다."
									  ,dto.getPlayerList().get(2), dto.getPlayerList().get(3), setCount);
							setCount++;
							setPointB++;
						}
					}
	...
	public void tennisRules() {
		String path2 = ".\\dat\\TennisRules.txt";
